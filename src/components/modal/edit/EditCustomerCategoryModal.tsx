...
const EditCustomerCategoryModal: React.FC<EditProps> = ({
    isOpen,
    selectedId,
    onClose,
    onSuccess,
}) => {
    const [formData, setFormData] = useState<CustomerCategoryData>({
        id: 0,
        note: "",
        name: "",
    });
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            setError("");
            try {
                const response = await httpGet(endpointUrlv2(`master/customer-category/${selectedId}`), true);
                ...
                toast.error(err?.response?.data?.message || "Failed to fetch customer category data.");
                setError("Could not load customer category data.");
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleSubmit = async () => {
        ...
        try {
            await httpPost(endpointUrlv2(`master/customer-category/${selectedId}/update`), payload, true);
            ...
            setError(error?.response?.data?.message || "Gagal mengubah kategori pelanggan");
        }
            ...
            <div className="no-scrollbar relative w-full max-w-[500px] overflow-y-auto rounded-3xl bg-white p-6 dark:bg-gray-900 lg:p-8">
                ...
                >
                    ...
                    <div className="space-y-5 px-2 pb-3">
                        <div>
                            <Label htmlFor="name">Nama Kategory</Label>
                            <Input
                                type="text"
                                id="name"
                                name="name"
